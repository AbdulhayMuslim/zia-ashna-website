...
export async function getPublishedPosts({ take, skip } = {}) {
  const safeTake = Number.isInteger(take)
...
export async function getRelatedPublishedPosts({
  ...
  if (!categoryId || !excludePostId) {
    return [];
  }

  const safeTake = Math.min(Math.max(Number(take) || 2, 1), 10);
...
export async function getHomepageContent() {
  const [hero, about, activity, history, contact, settings, posts] =
    await Promise.all([
      ...
      prisma.activitySection.findUnique({
        where: {
          id: 1,
        },
        include: {
      ...
      prisma.historySection.findUnique({
        where: {
          id: 1,
        },
        include: {
