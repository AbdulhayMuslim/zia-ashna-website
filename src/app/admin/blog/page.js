...
function EditPostDrawer({ editor, onClose, onSaved }) {
  ...
  const initialForm = {
    ...
    tagIds:
      post.tags?.map((entry) => entry.tagId ?? entry.tag?.id).filter(Boolean) ??
      [],
  };
        ...
        <Dialog.Content className="fixed inset-y-0 right-0 z-50 flex w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl outline-none dark:bg-gray-900 data-[state=open]:animate-in data-[state=open]:slide-in-from-right data-[state=closed]:animate-out data-[state=closed]:slide-out-to-right duration-300">
          <div className="flex shrink-0 items-start justify-between gap-4 border-b border-border bg-card px-5 py-4 dark:bg-gray-800 sm:px-6">
            ...
            <button
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl text-text transition hover:bg-brand-primary/10 hover:text-brand-primary dark:text-text-dark"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
          ...
          <div className="flex-1 space-y-6 overflow-y-auto p-5 sm:p-6">
            <section className="space-y-5 rounded-3xl border border-border bg-card p-5 dark:bg-gray-800">
              <div>
                <h3 className="font-semibold text-heading dark:text-heading-dark">
                  Post details
                </h3>
                <p className="mt-1 text-sm text-text dark:text-text-dark">
                  Update the title, URL slug, and summary.
                </p>
              </div>
              <InputField
              ...
              <div className="flex items-end gap-3">
                ...
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => update("slug", generateSlug(form.title))}
                >
                  Generate
                </Button>
              </div>
              <TextareaField
                id="drawer-post-excerpt"
                label="Excerpt"
                rows={4}
                value={form.excerpt}
            ...
            <section className="space-y-5 rounded-3xl border border-border bg-card p-5 dark:bg-gray-800">
              <div>
                ...
                <p className="mt-1 text-sm text-text dark:text-text-dark">
                  Edit the main body of your post.
                </p>
              </div>
              <RichTextEditor
                id="drawer-post-content"
                label="Blog Content"
                value={form.content}
            ...
            <section className="space-y-4 rounded-3xl border border-border bg-card p-5 dark:bg-gray-800">
              <div>
                <h3 className="flex items-center gap-2 font-semibold text-heading dark:text-heading-dark">
                  <Tags className="h-4 w-4 text-brand-primary" /> Tags
                </h3>
                <p className="mt-1 text-sm text-text dark:text-text-dark">
                  Select or remove tags for this post.
                </p>
              </div>
              {tags.length ? (
                ...
                <p className="rounded-2xl bg-background p-4 text-sm text-text dark:bg-gray-900 dark:text-text-dark">
                  No tags are available. Create tags from the Tags page first.
                </p>
              )}
            </section>
            ...
            <section className="space-y-5 rounded-3xl border border-border bg-card p-5 dark:bg-gray-800">
              <div>
                ...
                <p className="mt-1 text-sm text-text dark:text-text-dark">
                  Choose the image displayed with this post.
                </p>
              </div>
              <ImageUploadField
                id="drawer-post-image"
          ...
          <div className="flex shrink-0 flex-col-reverse gap-3 border-t border-border bg-card p-4 dark:bg-gray-800 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <Link
              ...
              className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl px-4 text-sm font-medium text-text transition hover:bg-brand-primary/10 hover:text-brand-primary dark:text-text-dark"
            >
          ...
          >
            ...
            {categories.map((category) => (
              ...
              >
                ...
                <Check
                  className={`h-4 w-4 ${value === category ? "opacity-100" : "opacity-0"}`}
                />
              </button>
            ))}
          </div>
...
function PostDetailsDialog({ post, onClose, onEdit, onDelete }) {
  ...
  const tags =
    post.tags?.map((entry) => entry.tag?.name ?? entry.name).filter(Boolean) ??
    [];
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 flex max-h-[90vh] w-[94vw] max-w-2xl -translate-x-1/2 -translate-y-1/2 flex-col overflow-hidden rounded-3xl border border-border bg-background shadow-2xl outline-none animate-in fade-in-0 zoom-in-95 dark:bg-gray-900">
          ...
          <div className="flex-1 space-y-5 overflow-y-auto p-5 sm:p-6">
            ...
            <div>
              <h2 className="font-heading text-2xl font-bold text-heading dark:text-heading-dark">
                {post.title}
              </h2>
              <p className="mt-2 break-all text-sm text-text-muted dark:text-text-muted-dark">
                /{post.slug}
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-full bg-muted px-3 py-1 text-xs text-text dark:bg-gray-800 dark:text-text-dark">
                {formatPostDate(post)}
              </span>
            ...
            {tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-border px-3 py-1 text-xs text-heading dark:text-heading-dark"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <section className="rounded-2xl border border-border bg-card p-4 dark:bg-gray-800">
              <h3 className="mb-2 text-sm font-semibold text-heading dark:text-heading-dark">
                Excerpt
              </h3>
              <p className="whitespace-pre-wrap text-sm leading-6 text-text dark:text-text-dark">
                {post.excerpt || "No excerpt provided."}
              </p>
            </section>

            <section className="rounded-2xl border border-border bg-card p-4 dark:bg-gray-800">
              <h3 className="mb-3 text-sm font-semibold text-heading dark:text-heading-dark">
                Content
              </h3>
              <div
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border bg-card p-4 dark:bg-gray-800 sm:px-6">
            ...
            >
              <Button type="button" variant="danger" leftIcon={Trash2}>
                Delete
              </Button>
            </ConfirmDialog>
          </div>
...
export default function BlogPage() {
  ...
  async function openEditor(row) {
    ...
    try {
      ...
      if (!postResponse.ok)
        throw new Error(postResult.message || "Unable to load post.");
      if (!categoryResponse.ok)
        throw new Error(categoryResult.message || "Unable to load categories.");
      if (!tagResponse.ok)
        throw new Error(tagResult.message || "Unable to load tags.");
      setEditor({
  ...
  async function togglePostStatus(post) {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.message || "Unable to update post status.");
      replace(result.data);
  ...
  function statusToggle(post) {
    ...
    return (
      ...
      >
        {isUpdating ? (
          <span className="inline-flex items-center rounded-full bg-muted px-3 py-1 text-xs text-text-muted">
        ...
        <div className="flex min-w-0 items-center gap-3">
          <div className="relative h-12 w-16 shrink-0 overflow-hidden rounded-xl bg-brand-primary/10">
            {row.featuredImage || row.image ? (
              <Image
                src={row.featuredImage || row.image}
                alt=""
                fill
                sizes="64px"
                className="object-cover"
          ...
          <div className="min-w-0 flex-1">
            <div
              title={row.title}
              className="truncate font-medium text-heading dark:text-heading-dark"
            >
              {shorten(row.title, 52)}
            </div>
          </div>
        </div>
      ),
...

    {
      ...
      render: (row) => (
        <span
          ...
          className="inline-block max-w-full truncate rounded-full bg-brand-primary/10 px-3 py-1 text-xs font-medium text-brand-primary"
        >
      ...
      render: (row) => (
        <div className="flex justify-end gap-1.5">
          <button
            ...
            disabled={openingEditor === row.id}
            aria-label={`Edit ${row.title}`}
            title="Edit post"
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-border text-text transition hover:border-brand-primary/40 hover:bg-brand-primary/10 hover:text-brand-primary disabled:opacity-60 dark:text-text-dark"
          >
            {openingEditor === row.id ? (
          ...
          >
            <button
              ...
              title="Delete post"
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-border text-red-500 transition hover:border-red-500/30 hover:bg-red-500/10"
            >
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:hidden">
          {loading ? (
            [...Array(3)].map((_, index) => (
              <div
                key={index}
                className="h-40 animate-pulse rounded-2xl border border-border bg-muted dark:bg-gray-800"
              />
            ))
            ...
            paginatedPosts.map((post) => (
              ...
              >
                <div className="flex min-w-0 items-start gap-3">
                  ...
                  <h3 className="line-clamp-2 min-h-10 min-w-0 flex-1 text-sm font-semibold leading-5 text-heading dark:text-heading-dark">
                    {post.title}
                  </h3>
                </div>

                <div className="flex items-center justify-between gap-2">
                  <span className="truncate rounded-full bg-brand-primary/10 px-2.5 py-1 text-xs font-medium text-brand-primary">
                    {post.category || "Uncategorized"}
                  </span>
                  <span className="whitespace-nowrap rounded-full bg-muted px-2.5 py-1 text-xs text-text dark:bg-gray-900 dark:text-text-dark">
                    {formatPostDate(post)}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between gap-2 border-t border-border pt-3">
                  ...
                  >
                    <button
                      ...
                      disabled={openingEditor === post.id}
                      aria-label={`Edit ${post.title}`}
                      title="Edit post"
                      className="flex h-9 w-9 items-center justify-center rounded-xl border border-border text-text transition hover:border-brand-primary/40 hover:bg-brand-primary/10 hover:text-brand-primary disabled:opacity-60 dark:text-text-dark"
                    >
                      {openingEditor === post.id ? (
                        <Loader2 className="h-4 w-4 animate-spin" />
                      ) : (
                        <Pencil className="h-4 w-4" />
                      )}
          ...
          {totalPages > 1 && (
            ...
            >
              <button
                ...
                className="flex h-11 w-11 items-center justify-center rounded-xl border border-brand-primary/20 bg-bg text-brand-primary duration-300 hover:bg-brand-primary hover:text-white disabled:pointer-events-none disabled:opacity-40 dark:bg-bg-dark dark:text-brand-secondary"
              >
                <ChevronLeft size={18} />
              </button>
              {Array.from({ length: totalPages }, (_, index) => index + 1).map(
                (page) => (
                  <button
                    ...
                    className={`h-11 w-11 rounded-xl border text-sm font-medium duration-300 ${visiblePage === page ? "border-brand-primary bg-brand-primary text-white" : "border-brand-primary/20 bg-bg text-brand-primary hover:bg-brand-primary hover:text-white dark:bg-bg-dark dark:text-brand-secondary"}`}
                  >
                    {page}
                  </button>
                ),
