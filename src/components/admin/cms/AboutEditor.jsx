...
function EmptyCollection({
  ...
  return (
    <div className="flex flex-col items-center rounded-2xl border border-dashed border-border px-6 py-10 text-center transition-colors hover:border-brand-primary">
      ...
      <h3 className="mt-4 font-semibold text-heading dark:text-heading-dark">
        {title}
      </h3>
      <p className="mt-1 max-w-sm text-sm text-text dark:text-text-dark">
        {description}
      </p>
      <Button
        type="button"
  ...
  return (
    <div className="mb-5 flex items-center justify-between gap-3 border-b border-border pb-4">
      <div className="min-w-0">
        <p className="truncate font-medium text-heading dark:text-heading-dark">
          {title}
        </p>
...
export default function AboutEditor() {
  ...
  async function save() {
    ...
    try {
      ...
      const result = await response.json();
      if (!response.ok)
        throw new Error(result.message || "Unable to save about content.");
      const next = {
        ...INITIAL_VALUE,
        ...result.data,
    ...
    <PageContainer>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <Card
            title="About content"
            description="Write the main introduction visitors see in your about section."
          ...
          <Card
            header={
              <CollectionHeader
                ...
                description="Add the key numbers and labels used to summarize your experience."
                actionLabel="Add experience"
                onAdd={() => addItem("experiences", { number: "", title: "" })}
              />
            }
          >
            {form.experiences.length ? (
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {form.experiences.map((item, index) => (
                  <div
                    key={item.id ?? index}
                    className="rounded-2xl border border-border bg-background p-5 dark:bg-gray-900"
                  >
                    <ItemHeading
                      title={item.title || `Experience ${index + 1}`}
                      meta={`Highlight ${index + 1} of ${form.experiences.length}`}
                      onRemove={() => removeItem("experiences", index)}
                      removeLabel={`Delete experience ${index + 1}`}
                    />
                    <div className="space-y-4">
                      <InputField
                        id={`experience-${index}-number`}
                        label="Number"
                        value={item.number ?? ""}
                        onChange={(event) =>
                          updateItem(
              ...
              <EmptyCollection
                icon={BriefcaseBusiness}
                title="No experience highlights yet"
                description="Add your first career number or achievement."
          ...
          <Card
            header={
              <CollectionHeader
                ...
                description="Add the roles, institutions, and years that make up your professional history."
                actionLabel="Add job"
                onAdd={() =>
                  addItem("jobExperiences", {
                    role: "",
                    institution: "",
                    year: "",
                  })
            ...
            {form.jobExperiences.length ? (
              <div className="grid gap-4 lg:grid-cols-2">
                {form.jobExperiences.map((item, index) => (
                  ...
                  >
                    <ItemHeading
                      title={item.role || `Job ${index + 1}`}
                      meta={`Position ${index + 1} of ${form.jobExperiences.length}`}
                      onRemove={() => removeItem("jobExperiences", index)}
                      removeLabel={`Delete job ${index + 1}`}
                    />
              ...
              <EmptyCollection
                icon={BriefcaseBusiness}
                title="No job experience yet"
                description="Add your first role, institution, and year."
          ...
          <Card
            header={
              <CollectionHeader
                icon={GraduationCap}
                title="Education"
                description="Keep your academic background clear and easy to scan."
          ...
          >
            {form.education.length ? (
                ...
                {form.education.map((item, index) => (
                  ...
                  >
                    <ItemHeading
                      ...
                      meta={`Entry ${index + 1} of ${form.education.length}`}
                      onRemove={() => removeItem("education", index)}
                      removeLabel={`Delete education ${index + 1}`}
                    />
                    <div className="grid gap-4 sm:grid-cols-2">
                      <InputField
              ...
              <EmptyCollection
                ...
                description="Add a degree, institution, and completion year."
          ...
          <Card
            header={
              <CollectionHeader
                icon={Award}
                title="Certificates"
                description="Add the names of certificates you have earned."
                actionLabel="Add certificate"
                onAdd={() => addItem("certificates", { name: "" })}
              />
            }
          >
            {form.certificates.length ? (
                ...
                {form.certificates.map((item, index) => (
                  ...
                  >
                    <ItemHeading
                      ...
                      meta={`Credential ${index + 1} of ${form.certificates.length}`}
                      onRemove={() => removeItem("certificates", index)}
                      removeLabel={`Delete certificate ${index + 1}`}
                    />
                    <InputField
              ...
              <EmptyCollection
                ...
                description="Add the name of your first certificate."
          ...
          <div className="sticky bottom-4 z-20 flex flex-col gap-3 rounded-2xl border border-border bg-card/95 p-4 shadow-xl backdrop-blur dark:bg-gray-800/95 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-text dark:text-text-dark">
              {dirty
                ? "You have unsaved about changes."
                : "All about changes are saved."}
            </p>
