          ...
          {data.addresses?.length > 0 && (
            <div className="grid w-full gap-3 sm:grid-cols-2">
              {data.addresses.map((address) => {
                ...
                const content = (
                  <>
                    ...
                    <span className="min-w-0">
                      <span className="block text-xs font-medium uppercase tracking-wide text-text-muted dark:text-text-muted-dark">
                        {address.label}
                      </span>
                      <span className="mt-1 block break-words text-sm font-medium text-heading dark:text-heading-dark">
                        {address.value}
                      </span>
                    </span>
                  </>
                ...
                const className =
                  "flex min-w-0 items-center gap-3 rounded-2xl border border-brand-primary/10 bg-card/80 p-3.5 text-start shadow-sm transition dark:border-white/10 dark:bg-white/5";
                return address.linkUrl ? (
                  <a
                    key={address.id}
                    href={address.linkUrl}
                    className={`${className} hover:-translate-y-0.5 hover:border-brand-primary/30 hover:shadow-md`}
                  >
                    {content}
                  </a>
                ) : (
                  <div key={address.id} className={className}>
                    {content}
                  </div>
                );
              })}
            ...
            <div className="w-full grid grid-cols-3 grid-rows-2 gap-4 place-items-center">
              {data.cards.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    className="flex h-full w-full flex-col items-center justify-center gap-2 rounded-xl bg-brand-primary/10 p-4 text-brand-primary"
                  >
                    <Icon />
                    <h4>{item.title}</h4>
                  </div>
